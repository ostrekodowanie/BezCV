...
export const inputStyles = {
    input: 'peer rounded-lg bg-[#FAFAFA] border-[#F5F5F6] border-[1px] focus:bg-[#F1F4FC] py-3 px-6 w-full transition-colors'
}
        ...
        <section className="padding py-[1.4in] md:py-[1.8in] xl:py-[2in] flex flex-col sm:gap-16 xl:grid xl:gap-32 xl:items-center grid-cols-[1fr_1fr]">
            <div className="flex flex-col">
                <h1 className='flex flex-col gap-2 mb-12'>
                    <span className='font-semibold text-[0.95rem]'>Skontaktuj się z nami</span>
                    <span className='text-3xl md:text-4xl font-bold'>za pomocą <span className="text-primary">formularza</span></span>
                </h1>
                <div className="bg-white rounded-xl xl:self-start w-full flex flex-col gap-6">
                    <form onSubmit={handleSubmit} className="flex flex-col sm:grid grid-cols-2 gap-8 max-w-full font-medium">
                        ...
                        <div className='relative flex flex-col gap-2 items-start w-full'>
                            <label className='text-sm' htmlFor="lastName">Nazwisko <span className="text-[#171A2380]">{'(opcjonalnie)'}</span></label>
            ...
            <img className="mt-24 mx-[12vw] xl:mx-0 xl:mt-0" src={contactMain} alt="" />
        </section>
    )
}