    ...
    return (
        <>
            <div className="flex flex-col items-stretch w-full gap-6 mt-8">
                {(type === 'checkbox' || type === 'radio') && answers?.map(ans => 
                    <label className={radioInputStyles} htmlFor={ans} key={'label:' + ans}>
                        <input value={ans} type={type} key={ans} id={ans} name={question} />
                        {ans}
                    </label>
                )}
                {type === 'range' && <>
                ...
                {(type === 'text' || type === 'email' || type === 'tel') && <input className={textInputStyles} value={answer} onChange={e => setAnswer(e.target.value)} placeholder={placeholder} type={type} />}
            </div>
            <div className="flex justify-between items-center gap-4 flex-wrap self-end">
                ...
                <button onClick={() => setQuestion(prev => prev + 1)} className="rounded-full text-[.8rem] text-white font-bold py-4 px-8 bg-secondary self-end flex items-center">Następne pytanie <img className="ml-2 max-h-[.9em]" src={buttonArrow} alt="" /></button>
            </div>
        </>
    )
}
