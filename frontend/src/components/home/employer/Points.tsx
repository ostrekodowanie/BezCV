    ...
    return (
        <section className="padding py-[1.4in] 2xl:py-[1.8in] bg-white min-h-screen">
            <div className='flex flex-col gap-4 mb-20'>
                <h2 className='text-3xl leading-snug md:text-4xl md:leading-snug font-semibold max-w-[5in] mb-4'>Rejestracja w BezCV jest <div className='inline-block relative font-bold'><span className='relative z-10'>zupełnie darmowa!</span><Control onScroll x={-30} duration={100} opacity={1} className="absolute -bottom-1 left-0 w-full" element={<img className="w-full" src={underline} alt="" />} /></div></h2>
                <p className='text-sm leading-loose font-medium text-[#3C4663] max-w-[7in]'>Dzięki temu możesz zobaczyć naszą bazę i sprawdzić, czy posiadamy kandydatów odpowiednich do pracy w Twoim przedsiębiorstwie.</p>
...
const Package = ({ points, price, image }: PackageProps) => {
    return (
        <div className="flex flex-col relative self-stretch h-full justify-end gap-4 rounded-3xl items-center p-12 bg-white shadow-primaryBig flex-1">
            ...
            {points === 20 && <img className='hidden xl:block absolute bottom-full left-[50%] -translate-x-[50%] max-w-[80%]' src={pointsMan} alt='' />}
        </div>
    )
}