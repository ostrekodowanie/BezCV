...
export default function WhatNext() {
    return (
        <section className="padding py-[1in] md:py-[1.4in] 2xl:py-[1.8in] items-center relative">
            <h2 className="text-center text-3xl xl:text-4xl font-bold mb-8">Co dalej?</h2>
            <Controller opacity={1} onScroll={true} stagger={80} ease='ease-out' className="flex flex-col gap-8 lg:grid grid-cols-3 xl:items-stretch">
                {boxes.map(box => <Control element={<Box {...box} />} key={box.image} />)}
            </Controller>
        </section>
    )
}
...
const Box = ({ image, desc }: BoxProps) => {
    return (
        <div className="rounded-xl shadow-secondaryBig px-6 py-8 md:px-10 md:py-12 flex flex-col gap-8 w-full h-full relative">
            ...
            <p className="font-medium text-[#444444]">{desc}</p>
