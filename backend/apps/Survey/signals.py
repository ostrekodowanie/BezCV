...
from .models import AbilityQuestions, CandidateAnswers, Questions
...
def update_percentage(sender, instance, **kwargs):
    ...
    questions = Questions.objects.filter(
        abilityquestions_question__in=ability_questions
    )
    answers = CandidateAnswers.objects.filter(
        candidate=candidate, question__in=questions
    )
    total_value = 0
