...
class PurchaseOfferView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if purchased_tokens > 0:
            purchased_tokens_obj = (
                user.purchasedpoints_employer.filter(
                    expiration_date__gt=timezone.now(), remaining_tokens__gt=0
                )
        ...
        client.sms.send(
            to=purchased_offer.candidate.phone,
            message=message,
            from_="bezCV",
            encoding="utf-8",
        )
...
class PurchasedOffersListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Candidates.objects.filter(
            purchasedoffers_candidate__employer=self.request.user
        )
...
def smsapi_endpoint(request):
    if request.method == "POST":
        ...
        if message == "1":
            ...
            return Response(
                {"status": "ERROR", "message": "Nieprawidłowa treść wiadomości."}
            )
