...
class CandidateSerializer(serializers.ModelSerializer):
    ...
    def get_ability_charts(self, obj):
        if obj.completed_surveys:
            abilities = (
                obj.candidateabilities_candidate.annotate(
                    category=F(
        ...
        if obj.completed_surveys:
            ...
            for ability in abilities:
                ...
                abilities_dict[category].append(
                    {"name": ability["name"], "percentage": ability["percentage"]}
                )
    ...
    def get_abilities(self, obj):
        if obj.completed_surveys:
            ...
            worst_abilities_names = [
                ability["name"]
                for category in worst_abilities.values()
                for ability in category
            ]

            abilities = self.get_abilities_helper(obj, Q(percentage__gte=20))

            all_abilities = (
                Abilities.objects.annotate(
            ...
            new_abilities = all_abilities.exclude(
                name__in=[
                    ability["name"]
                    for category in abilities.values()
                    for ability in category
                ]
                + worst_abilities_names
            )
            ...
            for ability in new_abilities:
                ...
                abilities[category].append(
                    {"name": ability["name"], "percentage": None}
                )
    ...
    def get_similar_candidates(self, obj):
        similar_candidates = (
            Candidates.objects.filter(profession=obj.profession)
            .exclude(id=obj.id)
            .values(
                "id",
                "first_name",
