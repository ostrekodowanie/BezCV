...
class Candidates(models.Model):
    ...
    contract_type = fields.ArrayField(
        models.CharField(
            max_length=255,
            choices=[
...
class CandidateAbilities(models.Model):
    candidate = models.ForeignKey(
        ...
        related_name="candidateabilities_candidate",
    )
    ...
    def __str__(self):
        return "{} | {}% | {}".format(self.pk, self.percentage, self.candidate.email)
...
class PurchasedOffers(models.Model):
    ...
    def __str__(self):
        return "{} | {} | {}".format(self.pk, self.created_at, self.points)


class Reports(models.Model):
    employer = models.ForeignKey(User, on_delete=models.CASCADE, related_name="reports")
    candidate = models.ForeignKey(
    ...
    def __str__(self):
        return "{} | {} | {}".format(
            self.employer.email, self.candidate.email, self.created_at
        )
...
class CandidateIndustries(models.Model):
    candidate = models.ForeignKey(
        Candidates,
        on_delete=models.CASCADE,
        related_name="candidateindustries_candidate",
    )
    industry = models.ForeignKey(
        Industries,
        on_delete=models.CASCADE,
        related_name="candidateindustries_industry",
    )
