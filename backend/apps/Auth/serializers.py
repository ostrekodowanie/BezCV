...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('first_name', 'last_name', 'points', 'is_staff',
                  'nip', 'company_name', 'image', 'desc', 'email', 'form')

    def get_points(self, obj):
        purchased_tokens = obj.purchasedpoints_employer.filter(expiration_date__gt=timezone.now(
        ), remaining_tokens__gt=0).aggregate(Sum('remaining_tokens'))['remaining_tokens__sum'] or 0
...
class EmployerProfileFollowedSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        purchased = instance.purchasedoffers_candidate.filter(
            employer=self.context['request'].user).exists()
        ...
        if not purchased:
            ...
            hidden_email = email_parts[0][0] + '*' * \
                (len(email_parts[0]) - 1) + '@' + email_parts[1]
